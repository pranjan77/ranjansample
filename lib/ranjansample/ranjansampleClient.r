...
ranjansampleClient <- function(url, token = NULL, flatten = TRUE, 
    ...
    ret_client_object_ref[['myfunc1']] <- function(wrkspace, myname) {
        ret <- ret_client_object_ref[['json_rpc_call']]("ranjansample.myfunc1", list(
                wrkspace, myname
        ))
        return(ret[[1]])
    }
    ...
    class(ret_client_object_ref) <- 'ranjansampleClient'
